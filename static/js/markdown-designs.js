function applyTailwind(container_id, dark_mode = false) {
    ...
    $(`${container} pre`).addClass(`w-full border my-4 p-4 rounded-md overflow-y-auto ${darkClass ? 'border-gray-600 bg-gray-700' : 'bg-gray-100'}`);
    ...
    $(`${container} code`).addClass(`p-1 rounded ${darkClass ? 'bg-gray-700 text-gray-300' : 'bg-gray-100'}`);
    $(`${container} hr`).addClass(`my-8 ${darkClass ? 'border-gray-600' : ''}`);
    $(`${container} table`).addClass(`w-full my-2 ${darkClass ? 'bg-gray-800' : ''}`);
    $(`${container} thead`).addClass(`bg-gray-50 rounded-lg border ${darkClass ? 'border-slate-600' : ''} font-normal ${darkClass ? 'bg-gray-700 text-gray-300' : ''}`);
